...
| `force_query_params` | Опционально. Принудительно правит выбранные query-параметры URI перед рендером: `fingerprint`, `alpn`, `allow_insecure`. Применяется только к TLS-подобным ссылкам; plain VLESS не меняется. |
